...
void json_object_put_format(struct json *json,
...
{
    bull_assert(json->type == JSON_STRING);
    return json->string;
}
...
{
    bull_assert(json->type == JSON_ARRAY);
...
{
    bull_assert(json->type == JSON_OBJECT);
...
{
    bull_assert(json->type == JSON_TRUE || json->type == JSON_FALSE);
...
{
    bull_assert(json->type == JSON_REAL || json->type == JSON_INTEGER);
...
{
    bull_assert(json->type == JSON_INTEGER);
    return json->integer;
}

...
json_parser_finish(struct json_parser *p)
{
    ...
    if (!p->error) {
        bull_assert(p->height == 1);
        bull_assert(p->stack[0].json != NULL);
