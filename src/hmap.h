...
static inline bool hmap_node_is_null(const struct hmap_node *node)
{
 ...
 * hmap_node_is_null().  */
static inline void hmap_node_nullify(struct hmap_node *node)
{
...
#define hmap_expand(HMAP) hmap_expand_at(HMAP, BULL_SOURCE_LOCATOR)

void hmap_shrink_at(struct hmap *, const char *where);
#define hmap_shrink(HMAP) hmap_shrink_at(HMAP, BULL_SOURCE_LOCATOR)

void hmap_reserve_at(struct hmap *, size_t capacity, const char *where);
#define hmap_reserve(HMAP, CAPACITY) \
    hmap_reserve_at(HMAP, CAPACITY, BULL_SOURCE_LOCATOR)

/* Insertion and deletion. */
static inline void hmap_insert_at(struct hmap *, struct hmap_node *, size_t hash, const char *where);
#define hmap_insert(HMAP, NODE, HASH) \
    hmap_insert_at(HMAP, NODE, HASH, BULL_SOURCE_LOCATOR)
...
static inline void hmap_replace(struct hmap *, const struct hmap_node *old, struct hmap_node *new_node);
...
static inline struct hmap_node *hmap_first_in_bucket(const struct hmap *, size_t hash);
...
struct hmap_node *hmap_at_position(const struct hmap *, struct hmap_position *);

/* Returns the number of nodes currently in 'hmap'. */
static inline size_t hmap_count(const struct hmap *hmap)
{
    return hmap->n;
}
 ...
 * rehashed. */
static inline size_t hmap_capacity(const struct hmap *hmap)
{
 ...
 * hmap_is_empty() is. */
static inline bool hmap_is_empty(const struct hmap *hmap)
{
    return hmap->n == 0;
}
...
static inline void hmap_replace(struct hmap *hmap, const struct hmap_node *old_node, struct hmap_node *new_node)
{
